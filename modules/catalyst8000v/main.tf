...
locals {
  c8k_user_data = templatefile("${path.module}/templates/c8k_config.tmpl", {
    cfg = var.cfg
  })
}
...
resource "google_compute_instance" "c8k_instance" {
  ...
  boot_disk {
    initialize_params {
      image = var.cfg.c8k.image_family
...
#resource "google_compute_instance_group_manager" "c8k_compute_instance_group_manager" {
#  name = "c8k-compute-instance-group-manager"
#
#  base_instance_name = var.cfg.c8k.hostname_prefix
